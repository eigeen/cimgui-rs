...
bitflags::bitflags! {
    ...
    pub struct ItemFlag: i32 {
        ...
        const SELECTABLE_DONT_CLOSE_POPUP = sys::ImGuiItemFlags_SelectableDontClosePopup;
        const MIXED_VALUE = sys::ImGuiItemFlags_MixedValue;
        const READ_ONLY = sys::ImGuiItemFlags_ReadOnly;
        const NO_WINDOW_HOVERABLE_CHECK = sys::ImGuiItemFlags_NoWindowHoverableCheck;
        const ALLOW_OVERLAP = sys::ImGuiItemFlags_AllowOverlap;
        const INPUTABLE = sys::ImGuiItemFlags_Inputable;
        const HAS_SELECTION_USER_DATA = sys::ImGuiItemFlags_HasSelectionUserData;
    }
}
